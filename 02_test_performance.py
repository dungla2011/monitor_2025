...
from test_env_loader import (
    ...
    get_test_api_url, get_performance_test_settings
)
...
class PerformanceTester:
    def __init__(self):
        self.api_base_url = get_test_api_url()
        self.results = {}
        self.perf_settings = get_performance_test_settings()
        
        print(f"🔧 Performance test API: {self.api_base_url}")
        print(f"🔧 Test iterations: {self.perf_settings['iterations']}")
        print(f"🔧 Concurrent users: {self.perf_settings['concurrent_users']}")
    ...
    def test_api_response_time(self, endpoint='/', iterations=None):
        """Test API response time"""
        if iterations is None:
            iterations = self.perf_settings['iterations']
    ...
    def test_concurrent_requests(self, endpoint='/', concurrent_users=None, requests_per_user=None):
        """Test concurrent API requests"""
        if concurrent_users is None:
            concurrent_users = self.perf_settings['concurrent_users']
        if requests_per_user is None:
            requests_per_user = self.perf_settings['iterations']
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--help':
        ...
        print("Usage:")
        print("  python 02_test_performance.py           - Run all performance tests")
        print("  python 02_test_performance.py --help    - Show this help")
        return
    
    # Check if service is running
    try:
        test_api_url = get_test_api_url()
        response = requests.get(f'{test_api_url}/api/status', timeout=5)
        if response.status_code != 200:
            print(f"❌ Monitor service is not running or not responding on {test_api_url}")
            print("   Please start the service first with test environment:")
            print("   1. Copy .env.test to .env")
            print("   2. python monitor_service.py start")
            sys.exit(1)
    except:
        test_api_url = get_test_api_url()
        print(f"❌ Cannot connect to monitor service on {test_api_url}")
        print("   Please start the service first with test environment:")
        print("   1. Copy .env.test to .env (or set TEST_MODE=true)")
        print("   2. python monitor_service.py start")
        sys.exit(1)
